...
export default function Sales() {
  ...
  const openEdit = async (orderId: number) => {
    ...
    const res = await fetch(`/api/trpc/sales.detail?input=${encodeURIComponent(JSON.stringify({ json: { orderId, page: 1, pageSize: 999 } }))}`, { headers: token ? { Authorization: `Bearer ${token}` } : {} });
